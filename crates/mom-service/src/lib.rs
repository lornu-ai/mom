...
mod tests {
    use super::*;
    use mom_core::{MemoryId, MemoryItem, MemoryKind, ScopeKey, Content, Query};
    ...
    // ============================================================================
    // US-3: List & Filter Memories Tests
    // ============================================================================

    #[test]
    fn test_query_kind_parsing_single() {
        // Test parsing single kind from string
        let query = Query {
            scope: ScopeKey {
                tenant_id: "test".to_string(),
                workspace_id: None,
        ...
        assert_eq!(query.kinds, Some(vec![MemoryKind::Event]));
    }

    #[test]
    fn test_query_kind_parsing_multiple() {
        // Test parsing multiple kinds
        let query = Query {
            scope: ScopeKey {
                tenant_id: "test".to_string(),
                workspace_id: None,
            ...
            text: String::new(),
            kinds: Some(vec![MemoryKind::Event, MemoryKind::Summary, MemoryKind::Fact]),
            tags_any: None,
            limit: 10,
            since_ms: None,
            until_ms: None,
        ...
        assert_eq!(query.kinds.as_ref().map(|v| v.len()), Some(3));
    }

    #[test]
    fn test_query_tag_parsing_single() {
        ...
        assert_eq!(query.tags_any, Some(vec!["important".to_string()]));
    }

    #[test]
    fn test_query_tag_parsing_multiple() {
        // Test parsing multiple tags (any match)
        let query = Query {
            scope: ScopeKey {
                tenant_id: "test".to_string(),
                workspace_id: None,
            ...
            text: String::new(),
            kinds: None,
            tags_any: Some(vec!["urgent".to_string(), "bug".to_string(), "deploy".to_string()]),
            limit: 10,
            since_ms: None,
            until_ms: None,
        };

        assert_eq!(query.tags_any.as_ref().map(|v| v.len()), Some(3));
        assert!(query.tags_any.as_ref().unwrap().contains(&"urgent".to_string()));
    }

    #[test]
    fn test_query_limit_validation_and_clamping() {
        // Test that limit is clamped to max 100
        let query_small = Query {
            scope: ScopeKey { tenant_id: "test".to_string(), workspace_id: None, project_id: None, agent_id: None, run_id: None },
            text: String::new(),
            kinds: None,
            tags_any: None,
            limit: 5, // Small limit
        ...
        assert_eq!(query_small.limit, 5);

        let query_max_safe = Query {
            scope: ScopeKey { tenant_id: "test".to_string(), workspace_id: None, project_id: None, agent_id: None, run_id: None },
            text: String::new(),
            kinds: None,
            tags_any: None,
        ...
        assert_eq!(query_max_safe.limit, 100);

        // Note: In actual list_memories handler, values > 100 would be clamped via .min(100)
    }

    #[test]
    fn test_query_timestamp_range_parsing() {
        // Test parsing time range filters
        ...
        let query = Query {
            ...
            tags_any: None,
            limit: 10,
            since_ms: Some(hour_ago),
            until_ms: Some(now),
        };

        assert_eq!(query.since_ms, Some(hour_ago));
        assert_eq!(query.until_ms, Some(now));
    }

    #[test]
    fn test_query_scope_key_parsing() {
        // Test parsing full scope hierarchy
        let query = Query {
            scope: ScopeKey {
                tenant_id: "acme".to_string(),
                workspace_id: Some("eng".to_string()),
                project_id: Some("api".to_string()),
                agent_id: Some("build-agent".to_string()),
        ...
        assert_eq!(query.scope.tenant_id, "acme");
        assert_eq!(query.scope.workspace_id, Some("eng".to_string()));
        assert_eq!(query.scope.project_id, Some("api".to_string()));
        assert_eq!(query.scope.agent_id, Some("build-agent".to_string()));
        assert_eq!(query.scope.run_id, Some("run-123".to_string()));
    }
    ...
    fn test_query_combined_filters() {
        ...
        let query = Query {
            scope: ScopeKey {
                ...
                workspace_id: Some("proj".to_string()),
                project_id: None,
                agent_id: None,
                run_id: None,
            },
            text: String::new(),
            kinds: Some(vec![MemoryKind::Event, MemoryKind::Summary]),
            tags_any: Some(vec!["production".to_string(), "critical".to_string()]),
            limit: 50,
            since_ms: Some(1609459200000),
            until_ms: Some(1609545600000),
        };

        assert_eq!(query.scope.tenant_id, "corp");
        assert_eq!(query.kinds.as_ref().map(|v| v.len()), Some(2));
        assert_eq!(query.tags_any.as_ref().map(|v| v.len()), Some(2));
        assert_eq!(query.limit, 50);
        assert!(query.since_ms.is_some());
        assert!(query.until_ms.is_some());
    }

    #[test]
    fn test_query_defaults_no_filters() {
        // Test Query with minimal/default filters
        let query = Query {
            scope: ScopeKey {
            ...
            text: String::new(),
            kinds: None, // No kind filter
            tags_any: None, // No tag filter
            limit: 10, // Default limit
            since_ms: None, // No time range
            until_ms: None,
        };

        assert_eq!(query.kinds, None);
        assert_eq!(query.tags_any, None);
        assert_eq!(query.limit, 10);
        assert_eq!(query.since_ms, None);
        assert_eq!(query.until_ms, None);
    }

    #[test]
    fn test_query_limit_default_10() {
    ...
    #[test]
    fn test_query_all_memory_kinds_filterable() {
        // Test that all MemoryKind variants can be filtered
        let all_kinds = vec![
            MemoryKind::Event,
            MemoryKind::Summary,
            MemoryKind::Fact,
        ...
        assert_eq!(query.kinds, Some(all_kinds));
    }

    #[test]
    fn test_query_tenant_isolation_required() {
        // Test that tenant_id is always present (required for isolation)
        let query = Query {
            scope: ScopeKey {
                tenant_id: "required-tenant".to_string(),
                workspace_id: None,
                project_id: None,
                agent_id: None,
                run_id: None,
        ...
        // tenant_id must always be present for queries
        assert!(!query.scope.tenant_id.is_empty());
    }
}
